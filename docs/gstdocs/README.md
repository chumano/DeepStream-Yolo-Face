...
# compile and install only gst-plugins-bad/onnx
meson setup builddir -Dbad=enabled -Dgst-plugins-bad:onnx=enabled

# setup with package-origin
meson setup builddir -Dpackage-origin=chumano -Dgst-plugins-ugly:gpl=enabled  --reconfigure

# setup with  x264
meson setup builddir -Dpackage-origin=chuno -Dgst-plugins-ugly:gpl=enabled  --reconfigure

ninja -C builddir -t targets | grep onnxgg
ninja -C builddir -t targets | grep x264
meson compile -C builddir gstonnx

# install everything that was built
meson setup --wipe --prefix=$HOME/.local builddir
meson setup --prefix=$HOME/.local builddir -Dbad=enabled -Dgst-plugins-bad:onnx=enabled -Dpackage-origin=chumano --reconfigure

meson install -C builddir
#  /home/chumano/.local/lib/x86_64-linux-gnu
...
ldconfig -p | grep libgstreamer-1.0.so.0
