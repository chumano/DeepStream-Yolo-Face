...
# ./deepstream-image-meta-test 0 file:///app/videos/friends_short.mp4
# gdb --args ./deepstream-image-meta-test 0 file:///app/videos/friends_short.mp4
 */
        ...
        while (usrMetaList != NULL) {
          ...
          if (usrMetaData->base_meta.meta_type == NVDS_CROP_IMAGE_META) {
            ...
            snprintf (fileObjNameString, FILE_NAME_SIZE, "%s_%d_%d_%d_%s.jpg",
                ...
                obj_meta->obj_label);
            /* Write to File */
            g_print("[INFO] Saving cropped object to %s\n", fileObjNameString);
             if (enc_jpeg_image->outBuffer && enc_jpeg_image->outLen > 0) {
              g_print("[DEBUG] outBuffer: %p, outLen: %zu\n", enc_jpeg_image->outBuffer, enc_jpeg_image->outLen);
...
pgie_src_pad_buffer_probe (GstPad * pad, GstPadProbeInfo * info, gpointer ctx)
{
  ...
  for (l_frame = batch_meta->frame_meta_list; l_frame != NULL;
    ...
    if(frame_count <= 10) {
      ...
      /* Main Function Call */
      g_print("[INFO] Queueing full frame for encoding: frame_number=%d, batch_id=%u\n", frame_number, frame_meta->batch_id);
      nvds_obj_enc_process (ctx, &frameData, ip_surf, NULL, frame_meta);
      g_print("[INFO] Full frame queued successfully: frame_number=%d, batch_id=%u\n", frame_number, frame_meta->batch_id);
    }
    ...
    for (l_obj = frame_meta->obj_meta_list; l_obj != NULL; l_obj = l_obj->next) {
      ...
      if ((obj_meta->class_id == PGIE_CLASS_ID_PERSON
        ...
        /*Main Function Call */
        //g_print("[INFO] Queueing object for encoding: frame_number=%d, batch_id=%u, obj_num=%d\n", frame_number, frame_meta->batch_id, objData.objNum);
        //nvds_obj_enc_process (ctx, &objData, ip_surf, obj_meta, frame_meta);
        //g_print("[INFO] Object queued successfully: frame_number=%d, batch_id=%u, obj_num=%d\n", frame_number, frame_meta->batch_id, objData.objNum);
      }
    }
  }
