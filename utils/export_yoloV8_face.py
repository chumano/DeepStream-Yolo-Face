...
import ultralytics.yolo.utils.tal as _m
...
class DeepStreamOutput(nn.Module):
    ...
    def forward(self, x):
        y = x[0].transpose(1, 2) # (B, N, 20)
        return y


def yolov8_face_export(weights, device, fuse=True):
    model = YOLO(weights)
...
def main(args):
    ...
    model = yolov8_face_export(args.weights, device)
    ...
    with torch.no_grad():
        output = model(onnx_input_im)
        print("Origin Model output structure:")
        print_output_structure(output)
